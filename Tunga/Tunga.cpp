...
LRESULT  SetKeyBoardTunga(int nCode,WPARAM wParam,LPARAM lParam)
{
			    ...
			    if( __isAlpha(ch)||wParam == VK_SPACE)   //---If key is pressed and is not a control key---//
					{                        
							if (((DWORD)lParam & 0x80000000)==0x000000000)  //---If key is pressed and is not a control key---//
							{
								if(ch == '&')
								{
									findKannadaCharacters('k');
									findKannadaCharacters('\\');
									findKannadaCharacters('x');
								}
								else if(ch == '%')
								{
									findKannadaCharacters('j');
									findKannadaCharacters('\\');
									findKannadaCharacters('z');
								}
								else
								{
...
BOOL EXPORTED_DLL_FUNCTION InstallKeyboardHookTungaExpEXP(HWND hWnd)
{
	InstallKeyboardHookTunga();

	return TRUE;
}

//---------------Insatalls-the-Hook---//---This-is -called-by-the-.exe---//
BOOL __stdcall InstallKeyboardHookTunga() 
{
          BOOL bSuccess = FALSE;
          if( !ghKeyHook) 
          {
...
BOOL  __stdcall DeInstallKeyboardHookTunga()
{
          if(ghKeyHook) 
          {
...
{       
		  ...
		  if(!(__isAlpha(ch))&&!(GetKeyState(VK_SHIFT))||isspace(ch))
		  {
          clearQueue();
          kFlag.f_bit = 0;
		  kFlag.f_pressed_Once = 0;
...
if(__isAlpha(ch)&&(ch!='f')&&(ch!='X')&&!(kFlag.o_bit))
copyLastChar0(ch);
return ;
}
...
int found = FALSE;
	...
	for(j=0;j<11;j++)
	{
		if(kFlag.previous_0 == charsAfterF[j].ch)
		{
			found = TRUE;
			while(charsAfterF[j].iVal[i]!=-1) 
			{
				addKey(charsAfterF[j].iVal[i]);
				i++;
			}
		}
	}
